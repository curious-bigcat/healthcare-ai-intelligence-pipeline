/*=============================================================================
  07 - CORTEX SEARCH SERVICES
  Healthcare AI Intelligence Pipeline

  Two search services for RAG over unstructured content:
    1. DOCUMENT_SEARCH — parsed medical documents (PDFs)
    2. AUDIO_SEARCH    — transcribed patient consultations (WAVs)
=============================================================================*/
...
-----------------------------------------------------------------------
-- 1. DOCUMENT SEARCH — over parsed & enriched PDF content
-----------------------------------------------------------------------
CREATE OR REPLACE CORTEX SEARCH SERVICE PROCESSED.DOCUMENT_SEARCH
  ON (
    SELECT
      DOC_ID,
      FILE_NAME,
      -- Combine parsed text + summary + key insights for rich search
      CONCAT(
  ...
  COMMENT = 'Cortex Search over AI-parsed medical documents (PDFs)';

-----------------------------------------------------------------------
-- 2. AUDIO SEARCH — over transcribed consultation recordings
-----------------------------------------------------------------------
CREATE OR REPLACE CORTEX SEARCH SERVICE PROCESSED.AUDIO_SEARCH
  ON (
    SELECT
      AUDIO_ID,
      FILE_NAME,
      -- Combine transcript + summary + consultation notes for rich search
      CONCAT(
  ...
  COMMENT = 'Cortex Search over transcribed patient consultations (WAV audio)';
